...
func align_to_floor(vector: Vector2) -> Vector2:
	...
	if is_on_floor():
		return vector.slide(floor_collision[SerialEnums.NORMAL]).normalized() * vector.length()
...
func serial_move_and_collide(rel_vec: Vector2, infinite_inertia: bool = true, exclude_raycast_shapes: bool = true, test_only: bool = false) -> Array:
